...
- API docs are generated by Doxygen from the `///` comments in
  `include/atpg/`, plus the narrative pages in `docs/*.dox`. `docs/Doxyfile`
  is a stock file generated by `doxygen -g`, templated with `@VAR@`
  placeholders that `docs/CMakeLists.txt` substitutes.
- Build them with `cmake --preset=docs && cmake --build build/docs --target
  docs`. `ATPG_BUILD_DOCS` is OFF by default, so Doxygen is not needed for an
  ordinary build.
- `WARN_AS_ERROR = FAIL_ON_WARNINGS`, so broken cross-references and malformed
  doc comments fail the build. Note this does *not* catch missing docs:
  `EXTRACT_ALL = YES` automatically disables `WARN_IF_UNDOCUMENTED`.
